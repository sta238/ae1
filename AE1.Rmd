...
```
`glimpse()` is a quick way to see the variable names in your data, get the data types of each column, and view the first few entries in the table. Make sure you u
...
Compute the mean and median of the exam. Are they equal? How do they compare? 
...
Plot the exam scores as a histogram and a KDE on the same plot. The steps are broken down below to add each layer to the plot, but some code needs to be filled in. Try commenting any code you fill in.
```{r layers, exercise=TRUE}
# Create a (named) plot of the scores in the gradebook data. Choose a theme.
plot <- 
...
```{r plots_continuous, echo=FALSE, exercise=TRUE}
tests_long <-
